...
static void genkey(u8 *rand, u8 idx, u8 *key)
{
	u8 *ans = ans_tbl[idx];
	u8 t0[10];
	int i;
...
static void gentabs(u8 *rand, u8 *key, u8 idx, u8 *ft, u8 *sb)
{
	ft[0] = sboxes[idx][key[1]] ^ sboxes[(idx+1)%8][rand[6]];
	ft[1] = sboxes[idx][key[3]] ^ sboxes[(idx+1)%8][rand[4]];
	ft[2] = sboxes[idx][key[0]] ^ sboxes[(idx+1)%8][rand[2]];
	ft[3] = sboxes[idx][key[5]] ^ sboxes[(idx+1)%8][rand[7]];
	ft[4] = sboxes[idx][key[4]] ^ sboxes[(idx+1)%8][rand[5]];
	ft[5] = sboxes[idx][key[2]] ^ sboxes[(idx+1)%8][rand[0]];
	ft[6] = sboxes[idx][rand[9]] ^ sboxes[(idx+1)%8][rand[3]];
	ft[7] = sboxes[idx][rand[8]] ^ sboxes[(idx+1)%8][rand[1]];
	
	sb[0] = sboxes[idx][key[5]] ^ sboxes[(idx+1)%8][rand[8]];
	sb[1] = sboxes[idx][key[0]] ^ sboxes[(idx+1)%8][rand[5]];
	sb[2] = sboxes[idx][key[2]] ^ sboxes[(idx+1)%8][rand[9]];
	sb[3] = sboxes[idx][key[3]] ^ sboxes[(idx+1)%8][rand[0]];
	sb[4] = sboxes[idx][key[1]] ^ sboxes[(idx+1)%8][rand[2]];
	sb[5] = sboxes[idx][key[4]] ^ sboxes[(idx+1)%8][rand[1]];
	sb[6] = sboxes[idx][rand[6]] ^ sboxes[(idx+1)%8][rand[4]];
	sb[7] = sboxes[idx][rand[7]] ^ sboxes[(idx+1)%8][rand[3]];
}
...
int main(int argc, char **argv)
{
	...
	wiimote_key k;
	u8 d[] = {0x98, 0xe2, 0x67, 0xe9, 0x8b, 0x97, 0x12, 0x77, 0x28, 0xa9, 0x11, 0x93, 0x9b, 0x7b, 0xbf, 0xae};

	wiimote_gen_key(&k, d);
	
	u8 ed[] = {0x5d, 0x7b, 0x81, 0x47, 0xf5, 0x30, 0xc8, 0x9a};

	wiimote_decrypt(&k, 8, ed, 0 );
